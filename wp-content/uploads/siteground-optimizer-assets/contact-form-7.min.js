(() => {
    "use strict";
    const c = a => Math.abs(parseInt(a, 10)),
        a = (b, a) => {
            const c = new Map([
                ["init", "init"],
            ...
            c.has(a) && (a = c.get(a)), Array.from(c.values()).includes(a) || (a = `custom-${a=(a=a.replace(/[^0-9a-z]+/i," ").trim()).replace(/\s+/,"-")}`);
            const d = b.getAttribute("data-status");
            return b.wpcf7.status = a, b.setAttribute("data-status", a), b.classList.add(a), d && d !== a && b.classList.remove(d), a
        },
        d = (a, b, c) => {
        ...
        b = b => {
            ...
            return j.reduceRight((a, b) => c => b(c, a), l => {
                let e, f, {
                    url: g,
                    path: b,
                    endpoint: h,
    ...
    function m(b) {
        ...
        const m = b,
            d = null === (j = b.wpcf7) || void 0 === j ? void 0 : j.schema;
        if (void 0 === d) return;
        if (null === (k = e.target) || void 0 === k || !k.closest(".wpcf7-form-control-wrap[data-name]")) return;
        if (null !== (h = e.target) && void 0 !== h && h.closest(".novalidate")) return;
        const c = new FormData,
            l = [];
        for (const a of m.querySelectorAll(".wpcf7-form-control-wrap"))
            if (!a.closest(".novalidate") && (a.querySelectorAll(":where( input, textarea, select ):enabled").forEach(a => {
                ...
                }), a.dataset.name && (l.push(a.dataset.name), a.setAttribute("data-under-validation", "1"), a.dataset.name === e.target.name.replace(/\[.*\]$/, "")))) break;
        d.rules = (null !== (i = d.rules) && void 0 !== i ? i : []).filter(a => {
            let {
                field: b
            } = a;
            return l.includes(b)
        });
        const n = b.getAttribute("data-status");
        Promise.resolve(a(b, "validating")).then(a => {
            if (void 0 !== swv) {
                const a = swv.validate(d, c, e);
                for (const [c, {
                        error: d
                    }] of a) f(b, c), void 0 !== d && g(b, c, d)
            }
        }).finally(() => {
            a(b, n), b.querySelectorAll(".wpcf7-form-control-wrap[data-under-validation]").forEach(a => {
    ...
    const g = (a, c, b) => {
            var d;
            const f = `${null===(d=a.wpcf7)||void 0===d?void 0:d.unitTag}-ve-${c}`.replaceAll(/[^0-9a-z_-]+/gi, ""),
                e = a.querySelector(`.wpcf7-form-control-wrap[data-name="${c}"] .wpcf7-form-control`);
            (() => {
                const c = document.createElement("li");
                c.setAttribute("id", f), e && e.id ? c.insertAdjacentHTML("beforeend", `<a href="#${e.id}">${b}</a>`) : c.insertAdjacentText("beforeend", b), a.wpcf7.parent.querySelector(".screen-reader-response ul").appendChild(c)
            })(), a.querySelectorAll(`.wpcf7-form-control-wrap[data-name="${c}"]`).forEach(d => {
        ...
        f = (a, d) => {
            var b, c;
            const e = `${null===(b=a.wpcf7)||void 0===b?void 0:b.unitTag}-ve-${d}`.replaceAll(/[^0-9a-z_-]+/gi, "");
            null === (c = a.wpcf7.parent.querySelector(`.screen-reader-response ul li#${e}`)) || void 0 === c || c.remove(), a.querySelectorAll(`.wpcf7-form-control-wrap[data-name="${d}"]`).forEach(a => {
                var b;
                null === (b = a.querySelector(".wpcf7-not-valid-tip")) || void 0 === b || b.remove(), a.querySelectorAll("[aria-invalid]").forEach(a => {
    ...
    function l(c) {
        ...
        if (wpcf7.blocked) return e(c), void a(c, "submitting");
        const i = new FormData(c);
        h.submitter && h.submitter.name && i.append(h.submitter.name, h.submitter.value);
        const f = {
        ...
        b({
            endpoint: `contact-forms/${c.wpcf7.id}/feedback`,
            method: "POST",
            body: i,
            wpcf7: {
                endpoint: "feedback",
            ...
            const e = a(c, b.status);
            return f.status = b.status, f.apiResponse = b, ["invalid", "unaccepted", "spam", "aborted"].includes(e) ? d(c, e, f) : ["sent", "failed"].includes(e) && d(c, `mail${e}`, f), d(c, "submit", f), b
        }).then(a => {
            a.posted_data_hash && (c.querySelector('input[name="_wpcf7_posted_data_hash"]').value = a.posted_data_hash), "mail_sent" === a.status && (c.reset(), c.wpcf7.resetOnMailSent = !0), a.invalid_fields && a.invalid_fields.forEach(a => {
                g(c, a.field, a.message)
            }), c.wpcf7.parent.querySelector('.screen-reader-response [role="status"]').insertAdjacentText("beforeend", a.message), c.querySelectorAll(".wpcf7-response-output").forEach(b => {
                b.innerText = a.message
            })
        }).catch(a => console.error(a))
    }
    b.use((b, c) => {
        if (b.wpcf7 && "feedback" === b.wpcf7.endpoint) {
            const {
                form: c,
                detail: f
            } = b.wpcf7;
            e(c), d(c, "beforesubmit", f), a(c, "submitting")
        }
        return c(b)
    });
    const e = a => {
        a.querySelectorAll(".wpcf7-form-control-wrap").forEach(b => {
            b.dataset.name && f(a, b.dataset.name)
    ...
    function k(c) {
        ...
        b({
            endpoint: `contact-forms/${c.wpcf7.id}/refill`,
            method: "GET",
            wpcf7: {
                endpoint: "refill",
                form: c,
            ...
            c.wpcf7.resetOnMailSent ? (delete c.wpcf7.resetOnMailSent, a(c, "mail_sent")) : a(c, "init"), e.apiResponse = b, d(c, "reset", e)
        }).catch(a => console.error(a))
    }
    b.use((b, c) => {
    ...
    function n(a) {
        ...
        a.wpcf7 = {
            ...
            pluginVersion: d.get("_wpcf7_version"),
            locale: d.get("_wpcf7_locale"),
            unitTag: d.get("_wpcf7_unit_tag"),
            containerPost: c(d.get("_wpcf7_container_post")),
            ...
            const b = (a, b) => {
                    const g = c(a.getAttribute("data-starting-value")),
            ...
            b.detail.apiResponse.captcha && i(a, b.detail.apiResponse.captcha), b.detail.apiResponse.quiz && h(a, b.detail.apiResponse.quiz)
    ...
    document.addEventListener("DOMContentLoaded", c => {
        ...
        const b = document.querySelectorAll(".wpcf7 > form");
        "function" == typeof b.forEach ? (wpcf7 = {
            init: n,
            submit: l,
            reset: k,
            validate: m,
            ...null !== (a = wpcf7) && void 0 !== a ? a : {}
        }, b.forEach(a => wpcf7.init(a))) : console.error("Your browser does not support NodeList.forEach().")
    })
})()